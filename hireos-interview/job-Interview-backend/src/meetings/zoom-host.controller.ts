...
export class ZoomHostController {
  constructor(private readonly zoom: ZoomHostService, private readonly rtms: RtmsService) {}
  ...
  @Post('rtms/start')
  @Header('Cache-Control', 'no-store')
  startRtms(@Req() req: { identity: Identity }, @Headers('x-hireos-zoom') header?: string, @Body() body?: { roundId?: string }) {
    this.mutation(header);
    const roundId = body?.roundId?.trim();
    if (!roundId) throw new BadRequestException({ code: 'ZOOM_ROUND_ID_REQUIRED' });
