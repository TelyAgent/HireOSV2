...
export class MeetingsService {
  ...
  // Host joins require a short-lived ZAK issued for the same Zoom user that owns the
  // meeting. Attendee joins remain available separately for non-host participants.
  attendeeConfig(meetingNumber: string, password: string, displayName: string) {
    return this.sign(meetingNumber, password, displayName, 0);
  }

  hostConfig(meetingNumber: string, password: string, displayName: string, zak: string) {
    if (!zak) throw new ServiceUnavailableException({ code: 'ZOOM_ZAK_REQUIRED' });
    return { ...this.sign(meetingNumber, password, displayName, 1), zak };
  }
