...
const config = { get: key => ({ NODE_ENV: 'development', ZOOM_DEV_ENABLED: 'true', ZM_RTMS_CLIENT: 'rtms-client-id', ZM_RTMS_SECRET: 'rtms-client-secret', ZOOM_OAUTH_PUBLIC_CLIENT_ID: 'public-id', ZOOM_OAUTH_CALLBACK_PORT: '50591', ZOOM_MEETING_SDK_CLIENT_ID: 'sdk-id', ZOOM_MEETING_SDK_CLIENT_SECRET: 'sdk-secret' })[key] };
...
test('PKCE callback, encryption, user isolation, host signing and duplicate create prevention', async () => {
  ...
  service.call = async url => {
    ...
    if (url.endsWith('/users/me/zak')) return { token: 'private-zak' };
  ...
  try {
    ...
    assert.equal(callback.hostname, '127.0.0.1'); assert.equal(callback.port, '50591');
...
test('missing OAuth client and production fail closed', async () => {
  const missingConfig = { get: key => key === 'ZOOM_OAUTH_PUBLIC_CLIENT_ID' || key === 'ZM_RTMS_CLIENT' ? '' : config.get(key) };
  const missing = new ZoomHostService(missingConfig, new MeetingsService(config));
  await assert.rejects(missing.authorize(identity), error => error.getResponse().code === 'ZOOM_OAUTH_CLIENT_ID_REQUIRED');
...
test('uncertain creation cannot create a second meeting on retry', async () => {
  ...
  let creates = 0;
  try {
    await service.save(service.owner(identity), { id: 'user', name: 'Host', email: 'host@example.com', clientId: 'public-id', access: 'access', refresh: 'refresh', expires: Date.now() + 3600000 });
